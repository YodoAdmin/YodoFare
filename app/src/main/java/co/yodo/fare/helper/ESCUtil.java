...
public class ESCUtil {
    ...
    public static byte[] parseData( ServerResponse response, String total, String balance ) {
        try {
            byte[] next2Line = ESCUtil.nextLine( 2 );
            byte[] title = response.getParams().getMerchant().getBytes( "gb2312" );

            next2Line = ESCUtil.nextLine( 2 );
            byte[] FocusBalance = ( "Balance: " + balance ).getBytes( "gb2312" );
            ...
            byte[][] cmdBytes = {
                    ...
                    center, boldOn, fontSize1Big, FocusOrderTotal, boldOff, nextLine,
                    center, fontSize1Big, FocusBalance, next2Line,
